...
class DeferralTest < ActiveSupport::TestCase
  ...
  test "logged-in and deferred_by user must have station and aircraft access on create" do
    restricted_user = users(:maaz)
    allowed_user = users(:chris)

    @deferral.user = restricted_user
    @deferral.performed_by = restricted_user
    assert_not @deferral.valid?
    refute_empty @deferral.errors[:user]
    refute_empty @deferral.errors[:performed_by]

    @deferral.user = allowed_user
    @deferral.performed_by = allowed_user
    assert @deferral.valid?
  end
  ...
  test "fault attributes are getting reverted before destroy" do
    @deferral.destroy
    assert @deferral.fault.active?
    assert_nil @deferral.fault.defer_reason
  end
  ...
  test "destroying deferral will revert fault attributes" do
    @deferral.destroy
    assert @deferral.fault.active?
    assert_nil @deferral.fault.defer_reason
  end
